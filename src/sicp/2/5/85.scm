...
(define (drop n)
  (let ((project-proc (get 'project (list (type-tag n)))))
    (if (not (eq? '() project-proc))
        (let ((n-projected (project-generic n)))
          (if (equ? (raise-generic n-projected) n)
              (drop n-projected)
              n))
        n)))
...
(define (project-generic n) (apply-generic 'project n))
...
(define (install-rational-package)
  ...
  (define (rational->real r) (make-real (exact->inexact (/ (numer r)
                                                           (denom r)))))
  (define (rational->integer r) (make-integer (floor (/ (numer r)
                                                        (denom r)))))
...
(define (install-real-package)
  ...
  (define (real->rational r)
    (let ((rational (rationalize (inexact->exact r) 1/100)))
...
(define (install-complex-package)
  ...
  (define (=zero?-complex z) (and (= (real-part z) 0)
                                  (= (imag-part z) 0)))
  (define (complex->real c) (make-real (real-part c)))
  ;; interface to rest of the system
...
(define real (make-real 3.0))
(define complex (make-complex-from-real-imag 3 0))
